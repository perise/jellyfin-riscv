...
namespace Jellyfin.Server
{
    ...
    public class CoreAppHost : ApplicationHost
    {
        ...
        protected override void RegisterServices(IServiceCollection serviceCollection)
        {
            ...
            // Wrap in try-catch: on platforms where libSkiaSharp is absent (e.g. linux-riscv64)
            // the SkiaEncoder static constructor throws TypeInitializationException before
            // IsNativeLibAvailable() can execute its own try-catch.
            bool useSkiaEncoder;
            try
            {
                useSkiaEncoder = SkiaEncoder.IsNativeLibAvailable();
            }
            catch (Exception)
            {
                useSkiaEncoder = false;
            }
